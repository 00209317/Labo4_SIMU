void createLocalA(Matrix &A,mesh m){
    float tao = m.getParameter(TAO)/8;

    A.at(0).at(0) += -tao; A.at(0).at(1) += tao;
    A.at(1).at(0) += -tao; A.at(1).at(1) += tao;
}
...
void createLocalC(Matrix &C,mesh m){
    float lamda = (m.getParameter(LAMDA)*2)/6;

    C.at(0).at(0) += -lamda; C.at(0).at(1) += lamda;
    C.at(1).at(0) += -lamda; C.at(1).at(1) += lamda;
}

void createLocalD(Matrix &D,mesh m){
    float l = m.getParameter(ELEMENT_LENGTH);
    float mu = m.getParameter(MU);
    D.at(0).at(0) += mu/l;  D.at(0).at(1) += -mu/l;
    D.at(1).at(0) += -mu/l;  D.at(1).at(1) += mu/l;
}

/*void createLocalE(Matrix &E,mesh m){
    float l = m.getParameter(ELEMENT_LENGTH);
    float israel = m.getParameter(ISRAEL);
    E.at(0).at(0) += (israel*l)/2;
    E.at(1).at(0) += (israel*l)/2;  
}*/
...
void createLocalG(Matrix &G,mesh m){
    float deltha = m.getParameter(DELTHA)/2;
    G.at(0).at(0) += -deltha;  G.at(0).at(1) += deltha;
    G.at(1).at(0) += -deltha;  G.at(1).at(1) += deltha;
}

/*void createLocalH(Matrix &H,mesh m){
    float l = m.getParameter(ELEMENT_LENGTH);
    float ene = m.getParameter(ENE);
    H.at(0).at(0) += (ene*l)/2;  
    H.at(1).at(0) += (ene*l)/2;  
}*/
...
Vector createLocalb(int element,mesh &m){
    ...
    float israel = m.getParameter(ISRAEL)/2, l = m.getParameter(ELEMENT_LENGTH), ene = m.getParameter(ENE)/2;
    
    b.push_back(israel*l); 
    b.push_back(israel*l); 
    b.push_back(ene*l); 
    b.push_back(ene*l);
